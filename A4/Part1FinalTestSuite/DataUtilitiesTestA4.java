...
public class DataUtilitiesTestA4 extends DataUtilities {
    ...
    public void setUp() throws Exception {
	    ...
	    //New mock values added 
	    // [ [5.9],[7.0]] , [3.2, 4.6]]
    	mockingContext5 = new Mockery();
        values5 = mockingContext5.mock(Values2D.class);
        mockingContext5.checking(new Expectations() {
            {
            	
            	//2 Columns and 2 Rows
                one(values5).getColumnCount();
                will(returnValue(2));
                
                one(values5).getRowCount();
                will(returnValue(2));
                
               
                one(values5).getValue(0, 0);
                will(returnValue(5.9));
                
                one(values5).getValue(0, 1);
                will(returnValue(7.0));

                
                
                one(values5).getValue(1, 0);
                will(returnValue(3.2));
                
                one(values5).getValue(1, 1);
                will(returnValue(4.6));
...
//New Test Cases Added to kill mutations
    
    //Column total for data table at column index 1
    @Test 
    public void calculateColumnTotalTwoParameters() {
		double actual = DataUtilities.calculateColumnTotal(values5, 1);
    ...
    //Column total for data table at column index 1
    @Test 
    public void calculateColumnTotalThreeParameters() {
    	int[] validRows = new int[]{0,1}; 
		double actual = DataUtilities.calculateColumnTotal(values5, 1, validRows);
		double expected =11.6;
        assertEquals(expected, actual, epsilon);
    }
    ...
    @Test 
    public void calculateRowTotalTwoParameters() {
		double actual = DataUtilities.calculateRowTotal(values5, 1);
		double expected = 7.8;
        assertEquals(expected, actual, epsilon);
    }
    ...
    //row total for data table at row index 1
    @Test 
    public void calculateRowTotalThreeParameters() {
    	int[] validColumns = new int[]{0,1}; 
		double actual = DataUtilities.calculateRowTotal(values5, 1, validColumns);
		double expected =7.8;
        assertEquals(expected, actual, epsilon);
    }


    //check equality for array of different length and with different value at same index
    @Test 
    public void equal_A_and_B_ArrayNotEqualLengthDifferentValueAtSameIndex() {
    ...
    //check equality for array of different length and with same value at same index 
    @Test 
    public void equal_A_and_B_ArrayNotEqualLengthSameValueAtSameIndex() {
    	double[][] a = new double[15][21];
    	double[][] b = new double[10][20];
    ...
    //check if two empty array are equal
    @Test 
    public void equal_A_and_B_EmptyArray() {
    	double[][] a = {};
    	double[][] b = {};
    ...
    //clone an 2d double array
    @Test 
    public void cloneArray() {
    	double[][] source = {{1.5, 2.7}, {5.4,6.23,7.2}, {9.5,56.6,2.8}};
    	double[][] actual = DataUtilities.clone(source);
    	double [][] expected = {{1.5, 2.7}, {5.4,6.23,7.2}, {9.5,56.6,2.8}};
    	assertArrayEquals(expected, actual);
    } 
    
    
    //calculateColumnTotal Test Cases. Two Parameters
    //data is null
    @Test (expected = IllegalArgumentException.class)
    public void calculateColumnTotalWithTwoParametersForNullData() {
    	DataUtilities.calculateColumnTotal(null, 1);    	
    }
    
    
    
    //calculate Column Total Test Cases. Three Parameters    
    //data is null
    @Test (expected = IllegalArgumentException.class)
    ...
    //calculate Row Total. Two Parameters    
    //data is null
    @Test (expected = IllegalArgumentException.class)
    public void calculateRowTotalWithTwoParametersForNullData() {
    	DataUtilities.calculateRowTotal(null, 1);    	
    }
        
    
    //calculate Row Total Test Cases. Three Parameters    
    //data is null
    @Test (expected = IllegalArgumentException.class)
    public void calculateRowTotalWithThreeParametersForNullData() {
    	DataUtilities.calculateRowTotal(null, 1, null);    	
    }
...
//Testing the functionalities of the method calculateRowTotal
  ...
  //Row total for data table at row index 1 that included valid column
    ...
    public void calculateRowTotalForInValidColumn() {
   	 	int[] validColumn = new int[]{0,1,2,5}; 
		double actual = DataUtilities.calculateRowTotal(values, 1, validColumn);
		double expected =15.4;
        assertEquals(expected, actual, epsilon);
    }
...
//NEW Test Cases to Kill Mutation
    
    @Test 
    public void createNewNumberArrayNullAr() {
    	assertArrayEquals("It is a Number", testerN2, DataUtilities.createNumberArray(dataN2));
    }
    @Test (expected = RuntimeException.class)
    public void createNewNumberArrayNull() {
    	assertArrayEquals(tester2, DataUtilities.createNumberArray(null));
    }


    @Test (expected = RuntimeException.class)
    public void createNewNumberArrayNull2D() {
    	assertArrayEquals(tester2D2, DataUtilities.createNumberArray(null));
    }
	...
	@Test //(expected = IllegalArgumentException.class)
  	public void getCumulativePercentagesNominal() {
			...
			Double num1 = 0.250; Double num2 = 0.500; Double num3 = 0.750; Double num4 = 1.000;
			assertEquals(num1, (Double) result2.getValue(0), e);
			assertEquals(num2, (Double) result2.getValue(1), e);
			assertEquals(num3, (Double) result2.getValue(2), e);
			assertEquals(num4, (Double) result2.getValue(3), e);
	}
	...
	public void getCumulativePercentagesSizeMinusOne() {
			double e = .0001d;
			KeyedValues result2 = DataUtilities.getCumulativePercentages(keyed_values2);
			Double num1 = 0.250; Double num2 = 0.500; Double num3 = 0.750;
			assertEquals(num1, (Double) result2.getValue(0), e);//, .000001d);
			assertEquals(num2, (Double) result2.getValue(1), e);//, .000001d);
			assertEquals(num3, (Double) result2.getValue(2), e);//, .000001d);
	}
